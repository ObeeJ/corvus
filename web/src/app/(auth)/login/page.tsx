...
export default function LoginPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-[16px] font-bold tracking-tight text-white/90">Sign In</h1>
        <p className="font-mono text-[11px] text-white/35 mt-1.5 tracking-wide">Welcome back to Corvus Intelligence.</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-3.5">
        ...
        >
          {isLoading ? "Signing in..." : <><span>Sign In</span><MoveRight className="w-3.5 h-3.5" /></>}
        </Button>
      </form>

      <p className="text-center font-mono text-[11px] text-white/25 tracking-wide">
        No account?{" "}
        <Link href="/signup" className="text-[#F97316]/70 hover:text-[#F97316] transition-colors">Sign up</Link>
      </p>
    </div>
  );
}
